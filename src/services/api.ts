...
export const APIService = {
	...
	getMovieCreditsByPerson: async (personId: number): Promise<Movie[]> => {
	...
	getTVCreditsByPerson: async (personId: number): Promise<TVShow[]> => {
...
const getMockData = (endpoint: string): unknown => {
	...
	const mockMovie: Movie = {
		...
		poster_path: null,
		backdrop_path: null,
		overview: "This is a mock overview because no TMDB API key is provided.",
		vote_average: 8.5,
	...
	if (
		...
		const mockRandomizerOffset = Math.floor(
			Math.random() * 1000000,
		);
		return {
			results: Array(10)
